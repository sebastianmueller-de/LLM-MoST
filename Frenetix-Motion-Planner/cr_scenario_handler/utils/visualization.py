...
def visualize_agent_at_timestep(scenario: Scenario, planning_problem: PlanningProblem, ego: DynamicObstacle,
    ...
    ego_params.vehicle_shape.occupancy.shape.facecolor = "#E37222"  # Change facecolor to blue
    ego_params.vehicle_shape.occupancy.shape.edgecolor = "#9C4100"  # Change edgecolor to dark blue
    ...
    if traj_set is not None and replanning_counter == 0:
        ...
        valid_step = int(len(valid_traj) / 2)
           ...
           # 构建 cost_data 字典
            ...
            for key, value_list in val.costMap.items():
                if isinstance(value_list, (list, tuple)):
                    ...
                    # 如果值是一个单一的数字（以防万一），则直接格式化
                    cost_data['costMap'][key] = round(value_list, 2)
                else:
                    # 处理其他非数字或非列表/元组的类型，例如保持原样或转换为字符串
            ...
            line, = plt.plot(val.cartesian.x, val.cartesian.y,
                            zorder=60, linewidth=2.5, alpha=1, picker=False)
        ...
        legend = plt.legend(legend_handles, legend_labels, title="Trajectory IDs", loc='best', fontsize='small')
        legend.set_zorder(100)
        merged_data = {}
        for trajectory_id, trajectory_info in all_trajectories_data.items():
            # 初始化该 trajectory_id 的合并数据
